...
def get_args_parser():
    ...
    parser.add_argument(
        "--val-batch-size", default=200, type=int, help="batchsize of validation set"
    )
    parser.add_argument(
        "--num-workers",
        default=8,
    ...
    parser.add_argument(
        "--load-reconstruct-checkpoint",
        type=str,
        default=None,
        help="Path to the reconstructed checkpoint.",
    )
    parser.add_argument(
        "--test-reconstruct-checkpoint",
        action="store_true",
        help="validate the reconstructed checkpoint.",
    )

    calibrate_mode_group = parser.add_mutually_exclusive_group()
    calibrate_mode_group.add_argument(
        "--calibrate", action="store_true", help="Calibrate the model"
    )
    calibrate_mode_group.add_argument(
    ...
    optimize_mode_group.add_argument(
        "--optimize", action="store_true", help="Optimize the model"
    )
    optimize_mode_group.add_argument(
        "--load-optimize-checkpoint",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--w_bit", type=int, default=argparse.SUPPRESS, help="bit-precision of weights"
    )
    parser.add_argument(
        "--a_bit",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=argparse.SUPPRESS,
        choices=["hessian_perturb", "mse", "mae"],
    ...
    parser.add_argument(
        ...
        type=str,
        default=argparse.SUPPRESS,
        choices=["hessian", "hessian_perturb", "mse", "mae"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=argparse.SUPPRESS,
        help="clamp percentile of mlp.fc2.",
    )
    return parser
...
def save_model(model, args, cfg, mode="calibrate"):
    ...
    if mode == "calibrate":
        ...
        auto_name = "{}_w{}_a{}_optimsize_{}_{}_{}{}.pth".format(
            ...
            cfg.optim_metric,
            cfg.optim_mode,
            "_recon" if args.reconstruct_mlp else "",
        )
...
def load_model(model, args, device, mode="calibrate"):
    ...
    ckpt_path = (
        ...
        if mode == "calibrate"
        else args.load_optimize_checkpoint
    )
...
def main(args):
    ...
    cfg.calib_batch_size = (
        args.calib_batch_size
        if hasattr(args, "calib_batch_size")
        else cfg.calib_batch_size
    )
    cfg.recon_metric = (
        args.recon_metric if hasattr(args, "recon_metric") else cfg.recon_metric
    )
    cfg.calib_metric = (
        args.calib_metric if hasattr(args, "calib_metric") else cfg.calib_metric
    )
    cfg.optim_metric = (
        args.optim_metric if hasattr(args, "optim_metric") else cfg.optim_metric
    )
    ...
    g = mydatasets.ViTImageNetLoaderGenerator(
        data_path, args.val_batch_size, args.num_workers, kwargs={"model": model}
    )
    ...
    if args.reconstruct_mlp:
        for name, module in model.named_modules():
            if name.split(".")[-1] == "mlp":
                module.act = nn.LeakyReLU(negative_slope=0.0005)
        if args.load_reconstruct_checkpoint is not None:
            logging.info(
                f"Restoring checkpoint from '{args.load_reconstruct_checkpoint}'"
            )
            ...
            calib_loader = g.calib_loader(
                num=cfg.optim_size, batch_size=cfg.optim_batch_size, seed=args.seed
            )

            logging.info(
                "{} - Start reconstructing MLP blocks ...".format(get_cur_time())
            )
            mlp_reconstructor = MLPReconstructor(
                model, full_model, calib_loader, metric=cfg.recon_metric, temp=cfg.temp
            )
            mlp_reconstructor.reconstruct_model(pct=cfg.pct)
            logging.info("{} - MLP reconstruction finished.".format(get_cur_time()))

            save_path = os.path.join(
                root_path, "{}_reconstructed.pth".format(args.model)
            )
            logging.info(f"Saving checkpoint to {save_path}")
            torch.save(model.state_dict(), save_path)
    ...
    reparam = (
        args.load_calibrate_checkpoint is None and args.load_optimize_checkpoint is None
    )
    logging.info(
        "Wraping quantiztion modules (reparam: {}, recon: {}) ...".format(
            reparam, args.reconstruct_mlp
        )
    )
    ...
    if not args.load_optimize_checkpoint:
        if args.load_calibrate_checkpoint:
            logging.info(
                f"Restoring checkpoint from '{args.load_calibrate_checkpoint}'"
            )
            ...
            logging.info(
                "{} - start {} guided calibration".format(
                    get_cur_time(), cfg.calib_metric
                )
            )
            calib_loader = g.calib_loader(
                num=cfg.calib_size, batch_size=cfg.calib_batch_size, seed=args.seed
            )
            ...
            model.to(device)
            logging.info(
                "{} - {} guided calibration finished.".format(
                    get_cur_time(), cfg.calib_metric
                )
            )
    ...
    if args.optimize:
        ...
        logging.info(
            "{} - start {} guided block reconstruction".format(
                get_cur_time(), cfg.optim_metric
            )
        )
        block_reconstructor = BlockReconstructor(
            ...
            full_model,
            calib_loader,
            metric=cfg.optim_metric,
            temp=cfg.temp,
            use_mean_hessian=cfg.use_mean_hessian,
        )
        ...
        logging.info(
            "{} - {} guided block reconstruction finished.".format(
                get_cur_time(), cfg.optim_metric
            )
        )
    ...
    if args.optimize or args.test_optimize_checkpoint:
        ...
        val_loss, val_prec1, val_prec5 = validate(
            calib_loader, model, criterion, print_freq=args.print_freq, device=device
        )
