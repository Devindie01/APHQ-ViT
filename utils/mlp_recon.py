...
def positive_percentile(tensor, pct):
    ...
    while tensor_too_large:
        try:
            ...
            t = t.view(-1)
            positive_mask = t > 0
            positive_tensor = torch.where(
                positive_mask, t, torch.tensor(float("nan")).to(t.device)
            )
...
def negative_percentile(tensor, pct):
    mini_batch_size = 1
    tensor_too_large = True
    while tensor_too_large:
        try:
            ...
            t = t.view(-1)
            negative_mask = t < 0
            negative_tensor = torch.where(
                negative_mask, t, torch.tensor(float("nan")).to(t.device)
            )
            sorted_tensor, _ = negative_tensor.sort(dim=0)
            tensor_too_large = False
        except:
            mini_batch_size *= 2
    counts = (~torch.isnan(sorted_tensor)).sum(dim=0, keepdim=True).float()
...
class MLPReconstructor(QuantCalibrator):
    ...
    def init_block_raw_inp_outp(self, block, device):
        ...
        hooks.append(
            block.mlp.fc1.register_forward_hook(self.single_input_forward_hook)
        )
        hooks.append(
            block.mlp.fc2.register_forward_hook(self.single_input_forward_hook)
        )
    ...
    def reconstruct_single_block(
        ...
        w_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            w_optimizer, T_max=iters, eta_min=0.0
        )
        loss_func = LossFunction(
            block, weight=2.0, rec_loss=self.metric, max_count=iters, p=p
        )
        for i in range(iters):
            ...
            if self.metric == "hessian_perturb":
                cur_grad = (
                    block.mlp.raw_grad.to(device)
                    if self.use_mean_hessian
                    else block.mlp.raw_grad[idx].to(device)
                )
            else:
                cur_grad = None
...
class LossFunction:
    ...
    def __call__(self, pred, tgt, grad=None, quant_out=None):
        ...
        if self.count == 1 or self.count % 500 == 0:
            print(
                "Total loss:\t{:.3f} (rec:{:.3f}, quant:{:.3f})\tcount={}".format(
